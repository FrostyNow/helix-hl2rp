...
if (SERVER) then
	...
	function ENT:Use(activator)
		...
		if bActive then
			self:EmitSound("ambient/fire/mtov_flame2.wav", 75, 100, 0.8)
			self.loopsound:Play()
		else
			self:EmitSound("ambient/fire/mtov_flame2.wav", 75, 250, 0.8)
			self.loopsound:Stop()
		end
	end
else
	...
	function ENT:DrawTranslucent()
		if self:GetNetVar("active") then
			local firepos = self:GetPos() + (self:GetUp() * -1)
			-- Fire
			if (ix_Fire_sprite and ix_Fire_sprite.fire) then
				render.SetMaterial(ix_Fire_sprite.fire)
				render.DrawBeam(
