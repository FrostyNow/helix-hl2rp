...
function ENT:Draw()
	...
	-- Use PVS and distance checking to skip expensive 3D2D rendering
	if (EyePos():DistToSqr(self:GetPos()) > MAX_LIGHT_DIST) then 
		return 
	end
	...
	cam.Start3D2D(self:GetPos() + self:GetForward() * 13.2 + self:GetRight() * 8.5 + self:GetUp() * 20, angle, 0.0364)
		...
		draw.SimpleText( L("assistanceTerminal"):upper(), "terminal_title", 248, 20, Color( 255, 255, 225, 50 + math.abs( math.sin( CurTime() ) ) * 300 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		...
		if ( self:GetNetVar( "alarm" ) ) then
			...
			draw.SimpleText( L("terminalOfficersDispatched"), "terminal_requestText", 248, 105, Color( 0, 25, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( L("terminalRequester", self:GetNetVar( "requester", L("terminalUnidentified") )), "terminal_requestText", 248, 140, Color( 255, 255, 0, 50 + math.abs( math.sin( CurTime() ) ) * 300 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( L("terminalRemainInPlace"), "terminal_requestText", 248, 175, Color( 255, 255, 0, 50 + math.abs( math.sin( CurTime() ) ) * 300 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		else
			...
			draw.SimpleText( L("terminalPressTo"), "terminal_infoText", 248, 375, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( L("terminalRequestAssistance"), "terminal_infoText", 248, 405, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end

		-- surface.SetMaterial(noise)
		-- surface.SetDrawColor(255, 255, 255, 1)
		-- local scroll = (CurTime() * 0.5) % 1
		-- surface.DrawTexturedRectUV(0, 0, 496, 502, 0, scroll, 1, scroll + 1)
