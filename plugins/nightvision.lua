...
if (CLIENT) then
	...
	local blockCmds = {
		...
		"nv_id_sens_darkness",
		"nv_etisd_status",
		"nv_etisd_sensitivity_range",
		"nv_isib_status",
		"nv_isib_sensitivity",
		"nv_aim_status",
		"nv_aim_range",
		"nv_fx_blur_status",
		"nv_fx_blur_intensity",
		"nv_fx_distort_status",
		"nv_fx_colormod_status",
		"nv_fx_goggle_status",
		"nv_fx_noise_status",
		"nv_fx_noise_variety",
		"nv_fx_bloom_status",
	...
	for _, v in ipairs(blockCmds) do
		concommand.Add(v, function()
			if (v == "nv_togg") then
				if (ix.command.list["NightVision"]) then
					ix.command.Send("NightVision")
				end
			elseif (v == "nv_reset_everything") then
				NV_ResetEverything()
			end
		end)
	end
