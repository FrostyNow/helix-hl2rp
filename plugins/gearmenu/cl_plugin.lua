...
function PANEL:RefreshGearInv()
    ...
    -- Scan ALL item instances for equipped items
    local equippedItems = {}
    ...
    for _, item in ipairs(equippedItems) do
        ...
        -- Fix: Scale down during drag to match inventory grid (64px)
        -- This ensures the inventory's preview highlight matches its own grid.
        icon.OnStartDragging = function(this)
            this:SetSize(item.width * 64, item.height * 64)
        end

        icon.OnStopDragging = function(this)
            this:SetSize(item.width * 80, item.height * 80)
        end
        ...
        if (item.exRender) then
            ...
            icon.ExtraPaint = function(this, panelX, panelY)
                ...
                if (exIcon) then
                    ...
                    ikon:renderIcon(renderKey, item.width, item.height, item:GetModel(), item.iconCam, nil, bg)
                end
            end
        else
...
hook.Add("CreateMenuButtons", "ixGearMenu", function(tabs)
    -- Hide default inventory tab (gear tab replaces it)
    tabs["inv"] = nil
