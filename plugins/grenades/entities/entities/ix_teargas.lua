			...
			for k, v in pairs( ents.FindInSphere( self:GetPos(), 230*perc ) ) do
				if (v.IsPlayer and v:IsPlayer()) then
					...
					if (tr.Entity == v and hook.Run("CanPlayerTearGassed", v) != false) then
						...
						local betterArmor = ix.plugin.Get("better_armor")
						local badair = ix.plugin.Get("badair")

						if (betterArmor and badair) then
							local char = v:GetCharacter()
							if (char) then
								local mask = badair:GetEquippedBadAirProtectionItem(char, function(item)
									return item.gasmask == true
								end)

								if (mask and v:GetNetVar("gasmask")) then
									if (!mask.requiresGasmaskFilter or (badair:HasItemFilterInstalled(mask) and badair:GetItemFilterDurability(mask) > 0)) then
										protected = true
									end
								end
							end
