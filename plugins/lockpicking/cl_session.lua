...
function Class:Stop(share, message)
	...
	-- Stop hooks
	if ( message )  then
		ix.util.NotifyLocalised("PLUGIN.Messages[msg]")
	end
...
function Class:Think()
	...
	self.MaxInnerLockAngle = self.MaxInnerLockAngle or hardMaxAngle
	...
	if (self.exceedMax) then
		if (self.InnerLockAngle <= unlockMaxAngle) then
			self.Success = true
	...
	if (not self.RotatingLock and not self.Success and not self.ChangingPin) then
		...
		self.PinAngle = math.deg(math.atan2(math.min(mY - ScrH() / 2, 0), mX - ScrW() / 2))
...
function PLUGIN:PlayerButtonDown(client, btn)
	...
	if (btn == KEY_D) then
		...
		if (session.LastRotating and CurTime() - session.LastRotating < spamTime + 0.08) then return end
