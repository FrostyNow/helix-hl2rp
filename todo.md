...
- 자동 음성 대사 플러그인: 활성화 시, 캐릭터가 피격당하거나 장전 시, 바로 옆에서 같은 세력 플레이어가 사망 시 등 상황에서 준비된 자동 대사 출력, sentences.txt 참조, b메뉴(bind가능)으로 수동 선택도 가능(상황별로 준비). 이때 목록은 cssource에서 대사 선택하듯이 하고, 마우스 옆 앞뒤키(혹은 방향키)로 다음 페이지 보기.